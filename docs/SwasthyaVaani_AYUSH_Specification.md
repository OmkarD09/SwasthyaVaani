...
> **Status:** Canonical AYUSH Domain Source of Truth  
...
> **Primary references:** Final `SwasthyaVaani_architecture.md`, updated `SwasthyaVaani_PRD.md`, `SwasthyaVaani_TRD.md`, `SwasthyaVaani_backend_schema.md`, `SwasthyaVaani_rules.md`, `SwasthyaVaani_appflow.md`, and `SwasthyaVaani_Current_Project_State.md`.  
...
AyushAssessmentModel persistence
AYUSH RAG grounding
Doctor AYUSH view
```

These capabilities remain backward-compatible with legacy clinical state consumers.
...
Where supported by patient age/life-stage data, the engine prefers deriving the relevant context automatically (`format_vaya_from_age`) upon session creation instead of asking the patient to repeat known demographic information.
...
The richer assessment model is implemented additively:
...
It operates alongside `ClinicalState.ayush` as the persisted AYUSH assessment record (`backend/app/models/ayush.py`).
...
Doctor retrieval exposes AYUSH through the patient dossier endpoint (`GET /api/v1/doctor/patients/{id}`).
...
```text
[x] AYUSH runs inside the shared adaptive engine
[x] No second AYUSH interview loop exists
[x] Ayurveda is explicitly identified as the first detailed AYUSH system
[x] Existing Prakriti/Vikriti/Agni/Koshtha behavior remains compatible
[x] Ahara-Vihara is structured
[x] Expanded Dashavidha dimensions can participate adaptively
[x] Expanded dimensions are not mandatory questions for every patient
[x] Known information is not repeatedly requested
[x] Patient-stated vs AI-inferred information is distinguishable
[x] Evidence can be traced where practical
[x] Uncertainty remains visible
[x] Safety always takes priority
[x] RAG remains bounded to knowledge grounding
[x] No autonomous diagnosis/treatment is introduced
[x] Doctor can review/edit/confirm AYUSH information
[x] Physician actions remain auditable
[x] Voice and text use the same reasoning pipeline
[x] Existing modern clinical tests remain green
[x] AYUSH-specific tests pass
```
