...
class MockSpeechProvider(AbstractSpeechProvider):
    ...
    async def transcribe_audio(
        ...
        text = (
            "मुझे 3 दिनों से तेज बुखार और खांसी है"
            if "hi" in lang.lower()
            else "I have had persistent chest pain and mild fever for 2 days"
        )
...
class SarvamSpeechProvider(AbstractSpeechProvider):
    """
    Official Sarvam AI Indic Speech Provider (Saaras ASR & Bulbul v3 TTS).
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[SarvamSpeechProvider] ASR error {response.status_code}: {response.text}")
                    return await self.fallback.transcribe_audio(audio_bytes, language_code)
        except Exception as e:
            print(f"[SarvamSpeechProvider] Connection error, using fallback: {e}")
            return await self.fallback.transcribe_audio(audio_bytes, language_code)
    ...
    async def text_to_speech(
        ...
        """Convert clinical question text to spoken Base64 audio using Sarvam Bulbul v3 TTS."""
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"[SarvamSpeechProvider] TTS HTTP {response.status_code}: {response.text}")
                    return None
        except Exception as e:
            print(f"[SarvamSpeechProvider] TTS error: {e}")
            return None
