...
class MockOCRProvider(AbstractOCRProvider):
    ...
    async def process_document(
        ...
        doc_type = "PRESCRIPTION" if any(w in lowered for w in ["presc", "rx", "medicine"]) else "LAB_REPORT"
        ...
        return OCRExtractionResult(
            ...
            review_status="PROCESSED" if not low_confidence else "NEEDS_REVIEW"
        )
...
class PaddleOCRProvider(AbstractOCRProvider):
    ...
    @classmethod
    def _normalize_legacy(cls, result: Any) -> Optional[List[Dict[str, Any]]]:
    ...
    @classmethod
    def _normalize_modern(cls, result: Any) -> Optional[List[Dict[str, Any]]]:
